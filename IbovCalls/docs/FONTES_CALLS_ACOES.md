...
- **03/ago/2026 (varredura completa, não via WhatsApp — pedido explícito "faça uma varredura" por todos os ativos do layout, motivado pelo usuário notar MELI34 com só 1 call)**: 82→83 ativos (TRIS3 chegou via outra sessão), +49 calls novas em 24 tickers, via **8 agentes de pesquisa em paralelo por lote de ~10 tickers** (não mais 6 — lotes menores, mais foco por agente). Destaque: **MELI34 saiu de 1 pra 6 calls** (JPMorgan, Goldman Sachs, Morgan Stanley, Citi, Safra + BTG já existente) — confirmou a suspeita do usuário. Outros com ganho grande: RADL3 (+3), NATU3 (+2, visões bull adicionadas a um dataset que só tinha neutro/bear), CSNA3 (+2), EZTC3 (+2), KLBN11 (+2). **Fontes que mais renderam nesta rodada** (por contagem de calls confirmadas): Seu Dinheiro e Money Times empataram na liderança, seguidos de InfoMoney; BPMoney e EuQueroInvestir tiveram bom retorno pra bancos específicos (BPMoney: Citi/3tentos, MELI34/Safra; EuQueroInvestir: Bradesco BBI/RD Saúde, BofA/Ultrapar, Safra/Klabin). Para MELI34 especificamente, fontes internacionais em inglês (dailypolitical.com, finance.yahoo.com, insidermonkey.com) foram necessárias — cobertura de banco sobre BDR geralmente é publicada em inglês primeiro (research sobre o ticker NASDAQ "MELI"), replicada depois (ou nunca) em português. **1 data corrigida**: GGBR4/Safra citava 21/jun (domingo) — ajustada pra sexta-feira anterior (19/jun), mesmo padrão de fallback já usado antes. **Decisão do usuário 03/ago/2026**: essa varredura completa (8 agentes em paralelo, todos os tickers do dataset) agora faz parte padrão de "atualizar tudo" — ver [[feedback_atualizar_todo_o_site]] — não precisa mais pedir explicitamente.
