...
Fonte primária : API B3  |  Fallback: lista local Jun/2025
"""
import base64, json
...
_B3_API = ("https://sistemaswebb3-listados.b3.com.br"
...
def _fetch_from_b3() -> Optional[list[dict]]:
    try:
        r = requests.get(_B3_API.format(token=_b3_token()),
                         headers={"User-Agent":"Mozilla/5.0"},timeout=20)
        r.raise_for_status()
        results = r.json().get("results",[])
        if not results: return None
        return [{"ticker":i["cod"].strip(),"name":i.get("asset","").strip(),
                 "weight":float(str(i.get("part","0") or "0").replace(",","."))} for i in results]
...
def sync_components() -> int:
    ...
    logger.success(f"Componentes sincronizados: {len(components)} ativos (anteriores desativados)")
