...
export default function AssetDetailPage() {
  ...
  const [flow1m, setFlow1m] = useState<{ net_value_delta: number; n_funds_buying: number; n_funds_selling: number; n_months: number; ref_date_to: string | null } | null>(null);
  const [flow3m, setFlow3m] = useState<{ net_value_delta: number; n_funds_buying: number; n_funds_selling: number; n_months: number; ref_date_to: string | null } | null>(null);
  const [flow6m, setFlow6m] = useState<{ net_value_delta: number; n_funds_buying: number; n_funds_selling: number; n_months: number; ref_date_to: string | null } | null>(null);
  ...
  useEffect(() => {
    ...
    api.getAssetFlowAccumulated(assetId, 1).then(setFlow1m);
  ...
  return (
    <div className="space-y-6">
      ...
      {(flow1m || flow3m || flow6m) && (
        <div>
          <h2 className="font-semibold mb-2">Fluxo Acumulado</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              { label: "Último mês", expected: 1, data: flow1m },
              { label: "Últimos 3 meses", expected: 3, data: flow3m },
              { label: "Últimos 6 meses", expected: 6, data: flow6m },
            ].map(({ label, expected, data }) => (
              <div key={label} className="smb-card p-4">
                <div className="text-xs mb-1" style={{ color: "var(--text3)" }}>
                  {label}
                  {data?.ref_date_to && <span> · até {fmtMonth(data.ref_date_to)}</span>}
                </div>
                {data && data.n_months > 0 ? (
                  <>
                    ...
                    <div className="text-xs mt-1" style={{ color: "var(--text3)" }}>
                      ...
                      {data.n_months < expected && ` (${data.n_months} meses com dados)`}
                    </div>
                  </>
                ) : (
